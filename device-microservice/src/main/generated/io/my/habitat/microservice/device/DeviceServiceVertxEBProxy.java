...
public class DeviceServiceVertxEBProxy implements DeviceService {
  ...
  public DeviceService retrieveDevice(String deviceId, Handler<AsyncResult<Device>> resultHandler) {
    ...
    _vertx.eventBus().<JsonObject>send(_address, _json, _deliveryOptions, res -> {
      if (res.failed()) {
        ...
        resultHandler.handle(Future.succeededFuture(res.result().body() == null ? null : new Device(res.result().body())));
                      }
    });
    return this;
  }
  ...
  public DeviceService retrieveDevicePrice(String deviceId, Handler<AsyncResult<JsonObject>> resultHandler) {
    ...
    _json.put("deviceId", deviceId);
    DeliveryOptions _deliveryOptions = (_options != null) ? new DeliveryOptions(_options) : new DeliveryOptions();
    _deliveryOptions.addHeader("action", "retrieveDevicePrice");
    _vertx.eventBus().<JsonObject>send(_address, _json, _deliveryOptions, res -> {
      if (res.failed()) {
        resultHandler.handle(Future.failedFuture(res.cause()));
      } else {
  ...
  public DeviceService retrieveAllDevices(Handler<AsyncResult<List<Device>>> resultHandler) {
    ...
    JsonObject _json = new JsonObject();
    DeliveryOptions _deliveryOptions = (_options != null) ? new DeliveryOptions(_options) : new DeliveryOptions();
    _deliveryOptions.addHeader("action", "retrieveAllDevices");
    _vertx.eventBus().<JsonArray>send(_address, _json, _deliveryOptions, res -> {
      if (res.failed()) {
        resultHandler.handle(Future.failedFuture(res.cause()));
      } else {
        resultHandler.handle(Future.succeededFuture(res.result().body().stream().map(o -> o instanceof Map ? new Device(new JsonObject((Map) o)) : new Device((JsonObject) o)).collect(Collectors.toList())));
      }
    });
    return this;
  }
  ...
  public DeviceService retrieveDevicesByPage(int page, Handler<AsyncResult<List<Device>>> resultHandler) {
    ...
    _json.put("page", page);
    DeliveryOptions _deliveryOptions = (_options != null) ? new DeliveryOptions(_options) : new DeliveryOptions();
    _deliveryOptions.addHeader("action", "retrieveDevicesByPage");
    _vertx.eventBus().<JsonArray>send(_address, _json, _deliveryOptions, res -> {
      if (res.failed()) {
        resultHandler.handle(Future.failedFuture(res.cause()));
      } else {
  ...
  public DeviceService deleteAllDevices(Handler<AsyncResult<Void>> resultHandler) {
    ...
    JsonObject _json = new JsonObject();
    DeliveryOptions _deliveryOptions = (_options != null) ? new DeliveryOptions(_options) : new DeliveryOptions();
    _deliveryOptions.addHeader("action", "deleteAllDevices");
    _vertx.eventBus().<Void>send(_address, _json, _deliveryOptions, res -> {
      if (res.failed()) {
        resultHandler.handle(Future.failedFuture(res.cause()));
      } else {
